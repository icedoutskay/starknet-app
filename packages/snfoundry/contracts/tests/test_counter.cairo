...
fn test_increase_counter() {
    ...
    // Now stop cheat caller if needed
...
fn test_set_counter_owner() {
    ...
    assert!(dispatcher.get_counter() == new_counter, "The owner is unable to reset counter");
...
fn test_reset_counter_success() {
    ...
    assert!(erc_20.balance_of(user) == strk_to_fri(9), "Balance not deducted conrectly");
    assert!(
        erc_20.balance_of(owner_address()) == strk_to_fri(1), "Balance not increased conrectly",
    );
}